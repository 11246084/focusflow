...
> 2026-09-13 文件整理：路徑已更新；功能摘要見 [狀態總表](../README.md)。以下原有日期與驗收結論保留，不代表本次重新執行測試。
...
| 檔案 | 用途 |
|------|------|
| `datasets/FocusFlow_QA評測_AI入門基礎課_v1.xlsx` | 主評分表（50 題）＋評分準則＋統計儀表 |
| `datasets/question-bank.json` | 同一份題庫的機器可讀版本，供 runner 使用 |
...
| `reports/2026-08-24-評測結果.md` | **首輪基準線報告**（50 題、加權 4.35/5），之後調整後的對比基準 |
| `reports/2026-09-03-第二輪評測結果.md` | 第二輪報告（加權 4.68/5），prompt 修正後的對比 |
...
| 日期 | 改了什麼 | 針對哪個弱點 |
|------|----------|--------------|
| 2026-08-24 | 首輪基準線，加權 4.35/5 | — |
| 2026-08-30 | `answerGeneration.service.js` prompt 規則 7 → 11 條（整合全部片段、允許部分回答、時間定位題以時間區間為主體、專有名詞寫正確名稱、禁止口頭數數與自相矛盾） | C 完整性 3.62、5 題假拒答、時間定位題平均 3.30、STT 對齊未生效 |
| 2026-09-03 | **第二輪評測**，加權 4.35 → **4.68**。時間定位 3.30 → 4.81、C 完整性 3.62 → 4.30、假拒答 5 → 1 題 | 見 `reports/2026-09-03-第二輪評測結果.md` |

未改檢索：`QA_MATCH_LIMIT` 仍是 15，memory 模式仍無相關性門檻。

2026-09-03 第二輪的結論是**剩餘缺口已從生成端移到檢索端**：D05 仍拒答（未命中 V6 1055-1097s），
D01 與 M04 也被查出相關內容根本沒進 top-15（M04 的珍奶例子切在片段邊界外）。
下一輪要動的是檢索覆蓋，不是 prompt，也不是 `QA_MATCH_LIMIT`。
