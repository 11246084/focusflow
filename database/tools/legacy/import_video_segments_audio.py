# ============================================================
# focusflow — 寫入 video_segments_audio
# 執行方式：python database/tools/legacy/import_video_segments_audio.py
# 資料來源：STT_Whisper/data/outputs/embeddings_audio_gemini.jsonl
#
# upsert key：segment_id
# ============================================================
...
SOURCE_PATH = PROJECT_ROOT / "STT_Whisper" / "data" / "outputs" / "embeddings_audio_gemini.jsonl"

COLLECTION_NAME = "video_segments_audio"
...
# ============================================================
# 讀取 embeddings_audio_gemini.jsonl
# ============================================================
print(f"\n📂 讀取 {SOURCE_PATH}...")
...
with open(SOURCE_PATH, encoding="utf-8") as f:
    for line in f:
        ...
        if missing:
            print(f"  ⚠️  跳過（缺少欄位 {missing}）：{record.get('segment_id', '<unknown>')}")
            skip_count += 1
            continue

        embedding = record.get("embedding")
        if not isinstance(embedding, list) or not embedding:
            print(f"  ⚠️  跳過（embedding 為空）：{record.get('segment_id', '<unknown>')}")
            skip_count += 1
            continue
...
# ============================================================
# Upsert 寫入 video_segments_audio
# ============================================================
