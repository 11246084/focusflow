...
> 本章依 2026-09-14 本機 checkout 的 `package.json`、入口程式、部署 workflow 與服務程式整理。初評原稿圖保留供版面參考，**均為歷史圖／待重繪**，不代表目前部署或外部服務的驗收證據。
...
> **歷史圖／待重繪：**下圖含固定主機、ngrok 與舊版套件／流程描述；重繪時應改為上述邊界與環境可變項，避免把拓撲當成 live 證據。

![初評原稿第7章圖像：圖7-1-1-正式環境部署圖.png](../images/圖7-1-1-正式環境部署圖.png)

圖7-1-1　正式環境部署圖（初評歷史圖，待重繪）
...
> **歷史圖／待重繪：**下圖列出的前端版本與元件關係是初評快照；請依此節表格與 package manifest 重繪。

![初評原稿第7章圖像：圖7-2-1-前端套件圖.png](../images/圖7-2-1-前端套件圖.png)

圖7-2-1　前端套件圖（初評歷史圖，待重繪）

> **歷史圖／待重繪：**後端現有 batch、notification、avatar、FAQ、ShortAsset 與 processing webhook 模組，未完整反映於下圖。

![初評原稿第7章圖像：圖7-2-2-後端套件圖.png](../images/圖7-2-2-後端套件圖.png)

圖7-2-2　後端套件圖（初評歷史圖，待重繪）

> **歷史圖／待重繪：**Pipeline 目前有 run manifest、checkpoint、resume 與可選 parent hierarchy；下圖未呈現這些可追溯產物與 gate。

![初評原稿第7章圖像：圖7-2-3-AI-Pipeline套件圖.png](../images/圖7-2-3-AI-Pipeline套件圖.png)

圖7-2-3　AI Pipeline 套件圖（初評歷史圖，待重繪）

> **歷史圖／待重繪：**資料層需區分 Mongoose source-declared index、Atlas Vector Search 與 shared Atlas live state；下圖未區分。

![初評原稿第7章圖像：圖7-2-4-資料儲存套件圖.png](../images/圖7-2-4-資料儲存套件圖.png)

圖7-2-4　資料儲存套件圖（初評歷史圖，待重繪）

> **歷史圖／待重繪：**LINE／QA 圖不得把已存在的程式碼或過去 smoke 誤寫成當前 provider 正常；亦應加入 citation 與 no-answer 契約。

![初評原稿第7章圖像：圖7-2-5-LINE與QA套件圖.png](../images/圖7-2-5-LINE與QA套件圖.png)

圖7-2-5　LINE 與 QA 套件圖（初評歷史圖，待重繪）
...
> **歷史圖／待重繪：**下圖不足以表示目前前後端分層、batch 和外部整合邊界。

![初評原稿第7章圖像：圖7-3-1-系統元件圖.png](../images/圖7-3-1-系統元件圖.png)

圖7-3-1　系統元件圖（初評歷史圖，待重繪）

## 7-4 狀態機（State machine）或時序圖（Timing diagram）
...
| 對象 | 狀態／事件 | 實作約束 |
| --- | --- | --- |
| 登入 session | anonymous → authenticating → authenticated；401／403 → invalid | `/auth/me` 驗證 user 仍有效；network／5xx 為可重試錯誤，不應直接登出。 |
...
> **歷史圖／待重繪：**以下登入與註冊圖未反映 session restore、admin 分離入口及錯誤邊界。

![初評原稿第7章圖像：圖7-4-1-登入狀態機.png](../images/圖7-4-1-登入狀態機.png)

圖7-4-1　登入狀態機（初評歷史圖，待重繪）

![初評原稿第7章圖像：圖7-4-2-註冊狀態機.png](../images/圖7-4-2-註冊狀態機.png)

圖7-4-2　註冊狀態機（初評歷史圖，待重繪）

> **歷史圖／待重繪：**下圖應改為本節 Video processing 的合法轉換，並加上 batch aggregate 與 webhook secret 邊界。

![初評原稿第7章圖像：圖7-4-3-影片處理狀態機.png](../images/圖7-4-3-影片處理狀態機.png)

圖7-4-3　影片處理狀態機（初評歷史圖，待重繪）

> **歷史圖／待重繪：**下圖應加入 enrollment default-deny、FAQ cache、citation／answerStatus 及 no-answer。

![初評原稿第7章圖像：圖7-4-4-QA問答狀態機.png](../images/圖7-4-4-QA問答狀態機.png)

圖7-4-4　QA 問答狀態機（初評歷史圖，待重繪）

> **歷史圖／待重繪：**下圖應先表達 webhook 簽章與 bind token TTL，再進入共用 QA service。

![初評原稿第7章圖像：圖7-4-5-LINE-Bot對話狀態機.png](../images/圖7-4-5-LINE-Bot對話狀態機.png)

圖7-4-5　LINE Bot 對話狀態機（初評歷史圖，待重繪）
