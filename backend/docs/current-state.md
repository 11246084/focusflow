...
- auth / JWT / RBAC 主線已可用；login 必填預期 `role`，跨身分入口回 `403 ROLE_MISMATCH`；register 只開放 `student`（2026-09-18 起關閉 teacher 自助註冊，教師帳號由管理員建立），已涵蓋 duplicate index race、bcrypt 與 public-user 敏感欄位測試
