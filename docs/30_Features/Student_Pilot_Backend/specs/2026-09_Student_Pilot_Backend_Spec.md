...
LINE 不列入第 2 章的完整流程步驟表。

但 LINE 已具備獨立的多輪上下文機制，本輪必須修正三項缺口，

並以 P5 驗收。三項缺口見附錄 B，其中切換課程清除歷史見附錄 G.6。
...
| 2. 教師建立課程並上傳影片 | 半通 | 前端、後端 | 建課、上傳與後端交接尚未完整實測。前端多檔上傳仍依序呼叫單支介面。上傳完成後須取得 `youtubeVideoId` 並可播放，否則步驟 4（學生觀看影片）不成立。前端多檔上傳的介面優化不擋試用。 | 是（僅限 YouTube 回寫與播放） |
...
2. **測試腳本排除**：12＋2 題的測試腳本將 `TEST_0720` 的影片編號

   從清單中排除，確保正式證據只涵蓋 129 筆段落。排除只作用於測試腳本，

   不修改資料庫。

不採取的做法是將 `TEST_0720` 從課程中移除。該影片仍有 11 筆問題、1 筆對話、

5 筆訊息與 11 筆使用紀錄引用。移除會造成新的殘留資料，風險高於效益。
...
- 證據存於儲存庫的 `docs/30_Features/Student_Pilot_Backend/evidence/`，並同步歸檔至 Notion。
...
教師上傳影片後自動上傳至 YouTube 本輪納入，為教師上傳鏈路的必要環節，

不是本章排除項。`YOUTUBE_UPLOAD_ENABLED=true` 是硬性要求。
...
| YouTube 上傳失敗的自動復原常態啟用 | YouTube 上傳本身本輪必須完成，但本輪不增加自動復原的複雜度，失敗改由人工重試。 | `YOUTUBE_UPLOAD_RECOVERY_ENABLED=false` | 重試安全、過期上傳、重複上傳與人工介入均用正式帳號驗證 |
| YouTube 本地檔自動清理 | YouTube 上傳本身本輪必須完成，但第一階段一律保留本地原始檔，作為上傳失敗時的唯一救援來源。 | `YOUTUBE_UPLOAD_CLEANUP_ENABLED=false` | 遠端播放通過，另行核准清理策略、路徑、共享引用與回復方法 |
...
本輪新增影片一律以 `YOUTUBE_UPLOAD_ENABLED=true` 自動上傳至 YouTube，

但自動復原與本地檔自動清理維持 `false`，不會隨上傳功能一併啟用。
...
| 教師建課與上傳 | 用專用測試課程上傳一支小型正式測試影片 | 課程與影片編號可追蹤；錯誤有原因；不改 OpenCV 課程；上傳後 `videos` 文件須符合附錄 H.6 的回寫契約，且以測試學生帳號實際播放成功；未取得 `youtubeVideoId` 一律視為未通過 | 從上傳步驟重試，不刪課重建 |
...
- 「影像處理導論」的示範學生修課關係已撤銷。見附錄 H.7。
- 教師測試課程的建課、上傳與失敗重試至少實測一次。
- 教師測試課程新上傳的影片已取得 `youtubeVideoId`，且以測試學生帳號實際播放成功；上傳失敗時的處理方式已實測一次。
- LINE 已可連續追問並保留最近 4 輪，追問改寫已生效，且 P5 在多輪情境下通過。
- LINE 切換課程時會清除舊課程對話歷史，已實測一次。見附錄 G.6。
...
| 項目 | 目前處理 | 何時回頭看 |
|------|----------|------------|
| 多輪對話 4 輪上限是否合適 | 維持現值。組員對這個數字有疑慮，但目前沒有實測依據支持改成別的數字 | 取得 12＋2 題正式證據後，依實際追問行為判斷。附錄 I 的腳本會輸出網頁與 LINE 的輪數分布，作為調整依據。 |
| 102 筆殘留段落的成因 | 唯讀調查沒有找到成因，現行介面也無法清理 | P6 通過即代表不會被檢索到。成因留待日後有寫入授權時追查 |
| 影像處理導論仍是 `published` | 撤銷示範學生修課關係後已無帳號可存取，接受 | 試用結束後評估是否下架 |
...
- LINE 使用自己的對話狀態與歷史機制，資料存放在 `User` 文件的
  `lineConversationHistory`，與網頁的 `Conversation` 分離。

**LINE 多輪本輪必須修正的三項缺口**

1. 歷史上限目前硬編碼為 3 輪，須改為讀取既有的 `MAX_CONVERSATION_TURNS`，
   使 LINE 與網頁同為最近 4 輪。
2. LINE 追問目前未經過追問改寫，檢索使用追問原文。
   須由 LINE 端先把 `model` role 映射為 `assistant`，再重用既有追問改寫邏輯，
   將追問改寫為可獨立理解的問題後再檢索。不修改 contextualizer 本身。
3. 切換課程時未清除舊課程的 LINE 對話歷史，見附錄 G.6。

**LINE 本輪不做的部分**

LINE 與網頁共用同一份多輪對話紀錄，本輪不做。
此項不需要修改 `Conversation` 或 `Message` 的資料結構，
也不需要修改 `lineConversationHistory` 的 schema。
LINE 自己的多輪對話本輪必做，見上方三項缺口與附錄 G.6。
...
理由必須以結構化陣列保存，並可由讀取介面原樣取回，供後續優化腳本生成。
...
7. LINE 透過共用問答路徑使用與網頁相同的課程範圍規則，
   每一則問題都重新取得課程、檢查存取權並重建段落範圍，
   不沿用上一輪的檢索範圍。九項修正完成後 LINE 自動同步受益，
   不需要另外撰寫一套 LINE 專用的範圍檢查。
   本項為確認事項，不新增修正工作。
...
| P5 | LINE | 綁定 A 後問 B 的內容，或嘗試切到沒修的 B；在 A 課程連續追問三輪後，再詢問未修課程 B 的內容；另以有權存取的 B 課驗證由 A 切換到 B | 切課被拒絕或回無答案；多輪建立後仍不得出現未修 B 課的引用；合法切換到 B 後，A 課對話歷史已清除 |
...
### G.6 LINE 切換課程時的對話歷史清除

- **現況**：三條成功切換課程的路徑目前只更新 `activeCourseId`，沒有清除上一門課程的 `lineConversationHistory`。
- **風險**：檢索範圍會正確重建，不會取到其他課程的段落，但舊課程的對話內容仍會被送入回答模型。性質屬於跨課程內容外洩，不是回答品質問題。
- **本輪要求**：課程實際變更時必須清除 `lineConversationHistory`。若課程未變更，也就是重選同一門課，不需清除。
- **已有的正確行為**：權限失效時已會清除 `activeCourseId` 與 `lineConversationHistory`，此行為保留。
- 追問改寫必須使用清除後的歷史，不得以舊課程的對話內容改寫新課程的問題。
- 本項為本輪必做，隸屬 LINE 多輪範圍，不計入第 5 章的九項隔離修正，也不改變九項的編號與數量。
...
上傳失敗時保留本地原始檔與已完成的影片處理狀態，影片不得對學生顯示為可播放，
...
9. 記錄每次對話實際用到第幾輪，以及該輪是否使用追問改寫。
   ...
   網頁與 LINE 兩條通道分別統計。
   輸出併入既有的 JSON 結果檔，供試用後評估 4 輪上限是否需要調整。
...
保存位置是儲存庫的 `docs/30_Features/Student_Pilot_Backend/evidence/`，並同步歸檔至 Notion。
...
第五項是以 `YOUTUBE_UPLOAD_ENABLED=true` 完成正式上傳、`videos` 回寫、

測試學生播放，以及一次上傳失敗保留與僅重試上傳的驗收。

第六項是完成 LINE 多輪的三項修正與多輪情境下的 P5 驗收，

即歷史上限改為讀取設定、追問改寫、切換課程清除歷史。
...
本次再增加 1 至 2 人日，因為必須使用正式 YouTube 上傳設定驗證成功回寫與學生播放，

並實測一次失敗保留、人工重試及不重跑影片處理的邊界。

LINE 三項修正可重用既有的追問改寫邏輯，程式改動不大，

主要成本在新增測試與取得實測證據。不需要修改 `Conversation`、`Message`

或 `lineConversationHistory` 的資料結構。

LINE 三項修正與多輪 P5 驗收另計 1 至 2 人日。

因此總估時由 v1.0 的 7 至 11 人日，先因 YouTube 上傳鏈路增加 1 至 2 人日，

形成 8 至 13 人日的推導中間值，再因 LINE 多輪增加 1 至 2 人日，
...
**跨課程隔離 > 12＋2 題證據 > LINE 多輪三項修正 > 短影片審核介面 > 其他工作**。

跨課程隔離未完成時，不得開放學生試用。

其餘項目可如實標示並縮減 Demo 範圍。

若總工時超出可用時間，可延後的項目依序為：

1. 教師上傳失敗重試的第二次實測。
2. 短影片審核介面的前端串接。

跨課程隔離的九項修正、12＋2 題證據、LINE 多輪三項修正（含追問改寫），

以及 LINE 切換課程清除歷史不得延後。

其中跨課程隔離的九項修正與 12＋2 題證據是開放前提，

LINE 追問改寫決定多輪是否真的可用，

LINE 切換課程清除歷史屬跨課程內容外洩。
...
1. 教師上傳影片後自動上傳至 YouTube 納入本輪必做。
   `YOUTUBE_UPLOAD_ENABLED=true` 與 `QA_VECTOR_SEARCH_MODE=atlas`
   同列為兩項硬性要求。自動復原與本地檔自動清理維持 `false`。
   來源：2026-08-27 專題成員意見與後續唯讀盤點結果。
2. LINE 自己的多輪對話納入本輪必做，保留最近 4 輪。
   LINE 與網頁共用同一份對話紀錄本輪不做。
   來源：2026-08-27 專題成員意見與後續唯讀盤點結果。
3. LINE 切換課程時必須清除舊課程對話歷史，屬跨課程內容外洩，
   不得延後。見附錄 G.6。不計入第 5 章的九項隔離修正。
   來源：2026-08-27 專題成員意見與後續唯讀盤點結果。
4. LINE 追問改寫納入本輪必做，由 LINE 端映射 role 後重用既有邏輯，
...
| `YOUTUBE_UPLOAD_ENABLED` | `true` | 硬性要求。教師上傳影片後必須自動上傳至 YouTube 並回寫 `youtubeVideoId`，否則新影片無播放來源 |
| `YOUTUBE_UPLOAD_RECOVERY_ENABLED` | `false` | 本輪不做自動復原，失敗改由人工重試 |
...
4. 試用模式驗證通過後，記錄 Backend 十二項設定的實際生效值，
   ...
   與證據一併存入 `docs/30_Features/Student_Pilot_Backend/evidence/`。
