...
依系上規範，本章分兩節：8-1「資料庫關聯表」以集合關聯圖為主，每張圖之後以說明表逐條說明圖中連線的參照關係與限制（Constraints）；8-2「表格及其 Meta data」逐一列出每個集合的欄位、索引與資料契約。

**資料來源與驗證邊界。** 本章依 `backend/src/models/` 下 21 份 Mongoose schema、`backend/src/constants/enums.js`、`backend/src/scripts/` 與 `database/tools/setup/` 的索引與集合初始化腳本，以及示範資料服務說明各集合。文中描述為程式宣告的資料契約；MongoDB Atlas 的實查結果彙整於表8-2-29。
...
![MongoDB核心集合關聯圖](../images/圖8-1-1-MongoDB核心集合關聯圖-v4-1.png)

圖8-1-1 MongoDB核心集合關聯圖（分組總覽）

圖8-1-1 至圖8-1-5 是文件型資料庫的集合關聯圖，**不是 UML 類別圖，也不是關聯式資料庫的外鍵 ERD**。判讀方式如下：

1. **實線**：文件以 ObjectId 或字串識別欄位保存的參照。箭頭（圖8-1-1）或鴉爪符號（圖8-1-2 至圖8-1-5）由被參照方指向持有參照欄位的一方，連線標籤為持有方的欄位名稱。鴉爪符號的「‖」表示恰好一筆、「○‖」表示零或一筆、「○＜」表示零或多筆、「‖＜」表示一或多筆。
2. **虛線**：快照（回答或凍結當下複製進文件，不回查來源）或由命名規則推導的對應，兩者都沒有可回查的參照欄位。
3. **灰底方框**：在其他圖中定義的集合，只列主鍵以示連接點。
4. **欄位標記**：`PK` 主鍵、`UK` 唯一索引、`IDX` 一般索引、`TTL` 到期自動刪除、`ref` 應用層參照、`vector` 向量索引欄位、`TEXT` 全文索引。
5. **連線編號**：每條連線標有 ①、② 等編號，對應圖後說明表的同編號列，每條連線都有一列說明。

MongoDB **不提供外鍵與參照完整性檢查**。Mongoose 的 `ref` 只供 `populate()` 查詢時解析，資料庫不會阻止刪除被參照的文件，也不會連動刪除，因此以下各說明表的「維護層級」欄註明每條參照由哪一層保證：「資料庫層」表示有唯一、partial 或 TTL 索引把關，「應用層」表示由 service 維護。例如刪除影片時，`video.service.js` 會先清除該課程的 FAQ 快取，再刪除片段、刪除影片文件、移除相關通知，並從所有課程的 `videoIds[]` 移除該影片；使用紀錄與提問紀錄則刻意保留。

圖8-1-1 將 21 個集合分成五組。「帳號與課程」是其他各組的共同上游；「影片與知識片段」與「問答」「短影音」之間只有快照關係，使回答紀錄與腳本證據不因影片重新處理而改變。表8-1-1 逐條說明組與組之間的 9 條連線，並標出對應的細部圖與連線編號。

表8-1-1　圖8-1-1 分組連線說明

| 編號 | 連線（被參照 → 持有方） | 持有欄位 | 說明 | 細部連線 |
| --- | --- | --- | --- | --- |
| ① | 帳號與課程 → 問題回報 | `feedbacks.userId` | 每筆問題回報記錄回報者 | 圖8-1-2 ⑩ |
| ② | 帳號與課程 → 影片與知識片段 | `videos.courseId`、`uploadedBy`；`videobatches.courseId`、`createdBy`；兩類片段集合與 `clips` 的 `courseId` | 影片、批次與片段歸屬課程及上傳者 | 圖8-1-3 ①②③④⑦⑨⑬ |
| ③ | 影片與知識片段 → 帳號與課程 | `courses.videoIds[]`、`enrollments.watchedVideoIds[]` | 課程掛載的影片清單，以及學生已看完的影片 | 圖8-1-2 ⑧⑨ |
| ④ | 帳號與課程 → 問答、對話與使用紀錄 | 各集合的 `userId`、`courseId` | 對話、提問、使用紀錄與 FAQ 快取歸屬使用者與課程 | 圖8-1-4 ①②⑤⑥⑧⑨⑩ |
| ⑤ | 帳號與課程 → 通知與短影音 | `notifications.recipientId`、`createdBy`、`courseIds[]`；`shortscripts.courseId`、`createdBy`；`shortassets.courseId` | 通知的收件者、發送者與相關課程；腳本與成品歸屬課程 | 圖8-1-5 ①②③⑥⑦⑩ |
| ⑥ | 影片與知識片段 → 通知與短影音 | `notifications.videoId`、`shortassets.sourceVideoId` | 影片完成通知的深層連結；成品的素材影片 | 圖8-1-5 ④⑫ |
| ⑦ | 影片與知識片段 ⇢ 問答、對話與使用紀錄 | `questions.matches[]`、`messages.sources[]`、`faqs.matches[]` | 回答當下命中的片段快照 | 圖8-1-4 ⑪⑫⑬ |
| ⑧ | 影片與知識片段 ⇢ 通知與短影音 | `shortscripts.evidence[]` | 腳本凍結的證據片段快照 | 圖8-1-5 ⑨ |
| ⑨ | 問答、對話與使用紀錄 ⇢ 通知與短影音 | `shortscripts.sourceQuestions[]` | 由提問彙整的主題來源快照（只存文字與次數，不存提問識別碼） | 圖8-1-5 ⑧ |

### 帳號、課程與問題回報

![帳號課程與問題回報集合關聯圖](../images/圖8-1-2-帳號課程與問題回報集合關聯圖-v1-1.png)

圖8-1-2 帳號、課程與問題回報集合關聯圖

圖8-1-2 以 `users` 與 `courses` 為中心：頭貼與回報附件的影像本體各自獨立成集合，使身分驗證與回報列表查詢不必載入影像；`enrollments` 以複合唯一索引實現學生與課程的多對多關係。表8-1-2 逐條說明圖中 11 條連線。

表8-1-2　圖8-1-2 連線說明：參照、基數與限制

| 編號 | 連線（被參照 → 持有方） | 持有欄位 | 基數 | 限制（Constraints） | 維護層級 |
| --- | --- | --- | --- | --- | --- |
| ① | `users` → `avatars` | `avatars.userId` | 1 : 0..1 | 必填且唯一，一人至多一張頭貼（後寫入者覆蓋） | 資料庫層（唯一索引）＋應用層 |
| ② | `users` → `line_bind_tokens` | `line_bind_tokens.userId` | 1 : 0..* | 必填；`token` 唯一；`expiresAt` 設 TTL 索引，到期文件由資料庫自動刪除 | 資料庫層（唯一、TTL）＋應用層 |
| ③ | `users` → `courses` | `courses.teacherId` | 1 : 0..* | 必填；單欄索引供教師課程清單查詢 | 應用層 |
| ④ | `courses` → `users` | `users.activeCourseId` | 0..1 : 0..* | 選填，預設 `null`；LINE 切換課程時更新，撤銷該課程修課時以 `$unset` 移除 | 應用層 |
| ⑤ | `users` → `enrollments` | `enrollments.studentId` | 1 : 0..* | 必填；與 ⑥ 組成 `(studentId, courseId)` 複合唯一索引，一名學生對同一課程只有一筆關係 | 資料庫層（複合唯一）＋應用層 |
| ⑥ | `courses` → `enrollments` | `enrollments.courseId` | 1 : 0..* | 必填；撤銷以 `status = revoked` 表示而不刪除文件，重新指派可恢復進度 | 資料庫層（複合唯一）＋應用層 |
| ⑦ | `users` → `enrollments` | `enrollments.assignedBy`、`revokedBy` | 0..1 : 0..* | 選填，保留指派與撤銷的稽核資訊 | 應用層 |
| ⑧ | `videos` → `courses` | `courses.videoIds[]` | 0..* : 0..* | 有序陣列；一支影片可掛載到多門課程（`videos.courseId` 只記主課程）；上傳時以 `$addToSet` 加入，刪除影片時自所有課程 `$pull` | 應用層 |
| ⑨ | `videos` → `enrollments` | `enrollments.watchedVideoIds[]` | 0..* : 0..* | 元素為 ObjectId 但 schema **未宣告 `ref`**；用於計算觀看進度 | 應用層 |
| ⑩ | `users` → `feedbacks` | `feedbacks.userId` | 1 : 0..* | 必填並建索引；`role` 另存提交當下的角色快照 | 應用層 |
| ⑪ | `feedbacks` → `feedbackattachments` | `feedbackattachments.feedbackId`；反向 `feedbacks.attachments[].attachmentId` | 1 : 0..* | 雙向記錄；每筆回報最多 3 張、單張 10 MiB 由 API 層驗證；建立時依序寫入回報、附件、再回填附件清單，未使用交易 | 應用層 |

### 影片與知識片段

![影片與知識片段集合關聯圖](../images/圖8-1-3-影片與知識片段集合關聯圖-v1-1.png)

圖8-1-3 影片與知識片段集合關聯圖

圖8-1-3 的重點是片段如何指回影片：文字片段（Leaf）與上層片段（Parent）都以**字串** `videoId = String(videos._id)` 指回影片，而不是 `videos.videoId` 欄位；視覺片段則沒有參照欄位。表8-1-3 逐條說明圖中 13 條連線。

表8-1-3　圖8-1-3 連線說明：參照、基數與限制

| 編號 | 連線（被參照 → 持有方） | 持有欄位 | 基數 | 限制（Constraints） | 維護層級 |
| --- | --- | --- | --- | --- | --- |
| ① | `courses` → `videos` | `videos.courseId` | 1 : 0..* | 必填，記錄主課程；單欄索引，另與 `fileHash` 組複合索引供同課程重複偵測 | 應用層 |
| ② | `users` → `videos` | `videos.uploadedBy` | 1 : 0..* | 必填 | 應用層 |
| ③ | `courses` → `videobatches` | `videobatches.courseId` | 1 : 0..* | 必填；`(courseId, createdAt)` 複合索引；`batchId` 唯一 | 資料庫層（唯一）＋應用層 |
| ④ | `users` → `videobatches` | `videobatches.createdBy` | 1 : 0..* | 必填；`(createdBy, createdAt)` 複合索引 | 應用層 |
| ⑤ | `videos` → `videobatches` | `videobatches.items[].videoId` | 0..1 : 0..* | 影片建立成功後才寫入；單批影片數上限（10）由 API 層驗證，不在 schema 宣告 | 應用層 |
| ⑥ | `videos` → `video_segments_text` | `video_segments_text.videoId`（字串） | 1 : 0..* | 值為 `String(videos._id)`，與 ObjectId 跨型別橋接（見圖8-1-6）；單欄索引與 `(courseId, videoId)` 複合索引 | 應用層（跨型別橋接） |
| ⑦ | `courses` → `video_segments_text` | `video_segments_text.courseId` | 0..1 : 0..* | 選填，預設 `null`；QA 範圍過濾目前不依賴此欄位 | 應用層 |
| ⑧ | `videos` → `video_segments_parent` | `video_segments_parent.videoId`（字串） | 1 : 0..* | 必填；與 Leaf 同一命名空間 | 應用層 |
| ⑨ | `courses` → `video_segments_parent` | `video_segments_parent.courseId` | 1 : 0..* | 必填；`(courseId, videoId)` 複合索引 | 應用層 |
| ⑩ | `video_segments_parent` → `video_segments_text` | `childChunkIds[]` → Leaf 的 `chunkId` | Parent 對 Leaf 為 1..* | 必填且不得為空陣列；**順序是檢索契約的一部分**，不得排序或去重；`parentId` 唯一 | 應用層（含 schema validator） |
| ⑪ | `videos` ⇢ `video_segments_video` | 無參照欄位；`video_id`（`video_N`） | 0..* | 由 service 自影片的 `videoId`、`fileName`、`filePath`、`sourceUrl`、`videoUrl` 等欄位擷取 `video_N` 後比對 | 應用層（命名規則推導） |
| ⑫ | `video_segments_text` → `clips` | `clips.segmentId`（字串） | 0..1 : 0..1 | 必填且唯一；Leaf 的 `segmentId` 本身不唯一，屬 legacy 快取 | 資料庫層（唯一）＋應用層 |
| ⑬ | `courses` → `clips` | `clips.courseId` | 0..1 : 0..* | 選填 | 應用層 |

### 問答、對話與使用紀錄

![問答對話與使用紀錄集合關聯圖](../images/圖8-1-4-問答對話與使用紀錄集合關聯圖-v1-1.png)

圖8-1-4 問答、對話與使用紀錄集合關聯圖

圖8-1-4 有兩條並行的紀錄路徑：網頁多輪問答保存在 `conversations` → `messages`；每一次提問（網頁與 LINE）另寫入一筆 `questions`，並可對應到 `usage_logs` 的 `ask` 事件。三條虛線是片段快照。表8-1-4 逐條說明圖中 13 條連線。

表8-1-4　圖8-1-4 連線說明：參照、基數與限制

| 編號 | 連線（被參照 → 持有方） | 持有欄位 | 基數 | 限制（Constraints） | 維護層級 |
| --- | --- | --- | --- | --- | --- |
| ① | `users` → `conversations` | `conversations.userId` | 1 : 0..* | 必填並建索引 | 應用層 |
| ② | `courses` → `conversations` | `conversations.courseId` | 1 : 0..* | 必填並建索引；另有 `(userId, courseId, updatedAt)` 複合索引支援最近對話排序 | 應用層 |
| ③ | `conversations` → `messages` | `messages.conversationId` | 1 : 0..* | 必填並建索引；`(conversationId, createdAt)` 複合索引 | 應用層 |
| ④ | `messages` → `messages` | `messages.replyToMessageId` | 0..1 : 0..* | 同集合自參照，助理回覆指向被回覆的提問訊息；`(conversationId, replyToMessageId)` 複合索引 | 應用層 |
| ⑤ | `users` → `questions` | `questions.userId` | 1 : 0..* | 必填並建索引 | 應用層 |
| ⑥ | `courses` → `questions` | `questions.courseId` | 1 : 0..* | 必填並建索引；刪除影片或課程時提問紀錄保留 | 應用層 |
| ⑦ | `usage_logs` → `questions` | `questions.sourceUsageLogId` | 0..1 : 0..1 | unique＋sparse＋partial 索引（只索引 ObjectId 值），一筆使用紀錄至多對應一則提問 | 資料庫層（partial unique）＋應用層 |
| ⑧ | `users` → `usage_logs` | `usage_logs.userId` | 1 : 0..* | 必填並建索引 | 應用層 |
| ⑨ | `courses` → `usage_logs` | `usage_logs.courseId` | 0..1 : 0..* | 選填（登入事件不屬於任何課程）；**刻意不設 TTL** | 應用層 |
| ⑩ | `courses` → `faqs` | `faqs.courseId` | 1 : 0..* | 必填並建索引；`(courseId, normalizedQuestion)` 複合唯一索引 | 資料庫層（複合唯一）＋應用層 |
| ⑪ | `video_segments_text` ⇢ `questions` | `questions.matches[]`（`segmentId`、`videoId`、秒數等） | — | 回答當下的片段快照，不回查 | 快照 |
| ⑫ | `video_segments_text` ⇢ `messages` | `messages.sources[]`（`chunkId`、`videoId`、逐字稿等） | — | 同上 | 快照 |
| ⑬ | `video_segments_text` ⇢ `faqs` | `faqs.matches[]`（Mixed） | — | 保留問答回應完整形狀的快照 | 快照 |

### 通知與短影音

![通知與短影音集合關聯圖](../images/圖8-1-5-通知與短影音集合關聯圖-v1-1.png)

圖8-1-5 通知與短影音集合關聯圖

圖8-1-5 中，`notifications` 可選擇性地指回觸發它的影片或腳本，作為前端深層連結；短影音由 `shortscripts`（凍結證據與多版腳本）到 `shortassets`（成品審核與上架）形成可追溯鏈。表8-1-5 逐條說明圖中 12 條連線。

表8-1-5　圖8-1-5 連線說明：參照、基數與限制

| 編號 | 連線（被參照 → 持有方） | 持有欄位 | 基數 | 限制（Constraints） | 維護層級 |
| --- | --- | --- | --- | --- | --- |
| ① | `users` → `notifications` | `notifications.recipientId` | 1 : 0..* | 必填；三組複合索引的首欄；`(recipientId, dedupeKey)` partial unique 索引防止同一事件重複通知，系統公告不帶 `dedupeKey` 不參與 | 資料庫層（partial unique）＋應用層 |
| ② | `users` → `notifications` | `notifications.createdBy` | 0..1 : 0..* | 選填；系統自動通知為 `null` | 應用層 |
| ③ | `courses` → `notifications` | `notifications.courseIds[]` | 0..* : 0..* | 必填陣列，預設 `[]` | 應用層 |
| ④ | `videos` → `notifications` | `notifications.videoId` | 0..1 : 0..* | 選填；影片完成通知的深層連結；刪除影片時一併移除該影片的通知 | 應用層 |
| ⑤ | `shortscripts` → `notifications` | `notifications.scriptId` | 0..1 : 0..* | 選填；成品退回通知連回腳本 | 應用層 |
| ⑥ | `courses` → `shortscripts` | `shortscripts.courseId` | 1 : 0..* | 必填並建索引；`(courseId, topicKey)` 複合唯一索引，同一課程不重複產生相同主題 | 資料庫層（複合唯一）＋應用層 |
| ⑦ | `users` → `shortscripts` | `shortscripts.createdBy` | 0..1 : 0..* | 選填 | 應用層 |
| ⑧ | `questions` ⇢ `shortscripts` | `shortscripts.sourceQuestions[]` | — | 由提問彙整的文字、次數與人數快照，不存提問識別碼 | 快照 |
| ⑨ | `video_segments_text` ⇢ `shortscripts` | `shortscripts.evidence[]`（`chunkId`、`rawText` 等） | — | 凍結的證據快照；來源影片其後刪除或重新處理不影響 | 快照 |
| ⑩ | `courses` → `shortassets` | `shortassets.courseId` | 1 : 0..* | 必填；學生瀏覽複合索引的首欄；另存 `courseSnapshot` 課程快照 | 應用層 |
| ⑪ | `shortscripts` → `shortassets` | `shortassets.sourceScriptId`＋`sourceVersionNo` | 0..1 : 0..* | 兩者併用才能定位到腳本的特定版本；`sourceScriptId` 單欄索引 | 應用層 |
| ⑫ | `videos` → `shortassets` | `shortassets.sourceVideoId` | 0..1 : 0..* | 選填；`youtubeVideoId` 另有唯一＋sparse 索引 | 資料庫層（唯一）＋應用層 |

### schema 層限制彙整

表8-1-2 至表8-1-5 中「資料庫層」的限制，都來自 schema 宣告。表8-1-6 彙整本系統在 schema 層實際使用的限制種類；其餘規則（例如單批影片數、每日提問次數、附件數量與大小、角色權限）屬於 API 與 service 層的業務限制，不寫入資料庫綱要。

表8-1-6　schema 層宣告的限制種類與使用位置
...
問答檢索必須限定在學生有權存取的課程影片內。這個範圍的解析路徑橫跨 `courses`、`videos` 與三類片段集合，而且需要在 ObjectId 與字串之間轉換，是本系統資料設計中最容易被誤解之處。圖8-1-6 依 `backend/src/services/bridgeScope.service.js` 的實作畫出解析順序，以下依圖中編號①～⑦逐步說明。
...
3. **④ Parent（Gate 開啟時）**：以（`courseId` 等於該課程 **或** `videoId` 在允許集合內）且 `generationVersion` 為目前世代、`isActive = true` 過濾，再依 `childChunkIds[]` 回查 Leaf。此路徑受 `HIERARCHICAL_RETRIEVAL_ENABLED` 控制，預設關閉。
4. **⑤ 查詢 Leaf 片段**：`buildSegmentLookupQuery()` 產生 `{ videoId: { $in: allowedVideoIds } }`。`atlas` 模式把它當作 Vector Search 的 filter，並在取回結果後再以範圍過濾一次；`memory` 模式以它作為 `find` 條件，在程式內計算相似度。
5. **⑥ 視覺引用**：只在文字片段完全沒有命中時執行，自影片欄位擷取 `video_N` 後比對 `video_segments_video.video_id`。
6. **⑦ 回答紀錄**：Leaf 或視覺片段的命中結果以快照寫入 `questions.matches[]` 與 `messages.sources[]`（見表8-1-4 ⑪⑫）。
...
本系統以應用程式建立的影片（app-owned）**不寫入 `videos.videoId` 欄位**（`video.service.js` 的上傳與 YouTube 連結建立流程皆未設定），該欄位只出現在 Pipeline metadata 文件與示範資料上。因此若以 `videos.videoId` 去連接 `video_segments_text.videoId`，所有 app-owned 影片都會對不到任何片段。表8-1-7 整理各集合用來指向影片或片段的鍵，供跨集合查詢時對照。
...
- **表頭列與表尾列**：每張欄位表的前兩列為表頭，列出 Collection、中文名稱、DB 編號、主鍵、集合名稱來源與是否啟用 `timestamps`；最後一列「參照（應用層）」集中列出該集合持有的參照欄位（`欄位 → 集合.欄位`）與快照欄位。MongoDB 不宣告資料長度，也沒有外鍵，因此不設「長度」與「外鍵」欄。

所有集合的 `_id` 皆為 MongoDB 自動產生的 ObjectId 主鍵；啟用 `timestamps` 的集合另自動維護 `createdAt` 與 `updatedAt`，各表合併為一列。

### 綱要與命名規則

表8-2-1 為本章各集合共同遵循的綱要與命名規則，閱讀後續欄位表前可先對照。
...
**資料檔陳述**：記錄系統使用者帳號、角色、LINE 綁定與 LINE 對話狀態，以及進行中的忘記密碼驗證。

表8-2-3　`users` 欄位 Meta data

| Collection | `users` | 中文名稱 | 使用者 | DB 編號 | DB-01 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名（`User` → `users`） | **timestamps** | 啟用 |
...
| **參照（應用層）** | `activeCourseId` → `courses._id` |  |  |  |  |

### DB-02 avatars

**資料檔陳述**：頭貼影像本體，一名使用者至多一筆（後寫入者覆蓋）。獨立成集合是為了讓每次身分驗證、聚合與 `$lookup` 讀取使用者文件時不帶影像。

表8-2-4　`avatars` 欄位 Meta data

| Collection | `avatars` | 中文名稱 | 頭貼 | DB 編號 | DB-02 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `avatars` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `userId` → `users._id` |  |  |  |  |

### DB-03 courses

**資料檔陳述**：記錄教師建立的課程、課程狀態與掛載影片清單。

表8-2-5　`courses` 欄位 Meta data

| Collection | `courses` | 中文名稱 | 課程 | DB 編號 | DB-03 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `teacherId` → `users._id`；`videoIds[]` → `videos._id` |  |  |  |  |

### DB-04 enrollments

**資料檔陳述**：學生與課程的修課關係、觀看進度與指派／撤銷稽核。撤銷時保留文件並改 `status`，重新指派可恢復原有進度。

表8-2-6　`enrollments` 欄位 Meta data

| Collection | `enrollments` | 中文名稱 | 修課關係 | DB 編號 | DB-04 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `studentId`、`assignedBy`、`revokedBy` → `users._id`；`courseId` → `courses._id`；`watchedVideoIds[]` → `videos._id`（未宣告 `ref`） |  |  |  |  |

> 複合唯一索引 `(studentId, courseId)` 同時使「指派學生」的 upsert 在重試時具冪等性：重複指派會落在同一筆文件上，而不會產生重複關係。

### DB-05 videos

**資料檔陳述**：記錄 app-owned 課程影片的來源、檔案路徑、STT Pipeline 處理狀態與 YouTube 自動上傳狀態；同一集合也可能存在 Pipeline metadata 文件（見下方「混合集合邊界」）。

表8-2-7　`videos` 欄位 Meta data

| Collection | `videos` | 中文名稱 | 影片 | DB 編號 | DB-05 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`uploadedBy` → `users._id`。被參照：片段集合以 `String(_id)` 指向本集合（見圖8-1-6） |  |  |  |  |
...
**資料檔陳述**：教師一次上傳多支影片時的批次彙總與逐檔結果。

表8-2-8　`videobatches` 欄位 Meta data

| Collection | `videobatches` | 中文名稱 | 影片批次 | DB 編號 | DB-06 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名（`VideoBatch` → `videobatches`） | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`createdBy` → `users._id`；`items[].videoId` → `videos._id` |  |  |  |  |

> 批次狀態描述整體結果，個別影片另保有自己的處理狀態（`videos.processing`），因此部分成功的批次仍可逐檔觀察與重試。

### DB-07 video_segments_text

**資料檔陳述**：逐字稿切片（Leaf）、時間區間、文字向量與向量產生契約，是問答檢索的權威來源。文件由 Pipeline／匯入工具寫入，backend 主要讀取，並在刪除影片時依 `videoId` 刪除。

表8-2-9　`video_segments_text` 欄位 Meta data

| Collection | `video_segments_text` | 中文名稱 | 文字片段（Leaf） | DB 編號 | DB-07 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 環境變數 `VIDEO_SEGMENT_COLLECTION`（預設 `video_segments_text`，對齊 AI Pipeline） | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`videoId`（字串）＝ `String(videos._id)`，跨型別橋接 |  |  |  |  |
...
**資料檔陳述**：由多個 Leaf 依序聚合而成的上層片段（Parent）與其向量，供階層式檢索取得較大範圍的脈絡。Parent 與 Leaf 使用不同的識別碼命名空間，不共用集合。階層式檢索由 `HIERARCHICAL_RETRIEVAL_ENABLED` 控制，預設關閉。

表8-2-10　`video_segments_parent` 欄位 Meta data

| Collection | `video_segments_parent` | 中文名稱 | 上層片段（Parent） | DB 編號 | DB-08 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 環境變數 `VIDEO_SEGMENT_PARENT_COLLECTION`（預設 `video_segments_parent`） | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`videoId`（字串）＝ `String(videos._id)`；`childChunkIds[]` → `video_segments_text.chunkId` |  |  |  |  |
...
**資料檔陳述**：影像片段的檔案路徑、時間區間與視覺向量，目前只用於課程範圍內的視覺引用（文字片段無命中時的補充），**不是正式的多模態問答來源或精華片段發布來源**。此集合沿用 AI Pipeline 的 snake_case 欄位命名，是 21 個集合中唯一不採 camelCase 者，欄位不得與文字片段混用。

表8-2-11　`video_segments_video` 欄位 Meta data

| Collection | `video_segments_video` | 中文名稱 | 視覺片段 | DB 編號 | DB-09 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 環境變數 `VIDEO_SEGMENT_VIDEO_COLLECTION`（預設 `video_segments_video`） | **timestamps** | 未啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| **參照（應用層）** | 無參照欄位；`video_id` 以 `video_N` 命名規則對應 `videos`（由 service 推導） |  |  |  |  |

> 此集合 `timestamps: false`，不記錄 `createdAt` 與 `updatedAt`；文件由 Pipeline 整批寫入，時間資訊由執行產物側保存。

### DB-10 questions

**資料檔陳述**：每一次提問（網頁與 LINE）的問題、回答、命中片段快照與執行環境診斷。刪除影片或課程時刻意保留。

表8-2-12　`questions` 欄位 Meta data

| Collection | `questions` | 中文名稱 | 提問紀錄 | DB 編號 | DB-10 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `questions` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `userId` → `users._id`；`courseId` → `courses._id`；`sourceUsageLogId` → `usage_logs._id`。快照：`matches[]` |  |  |  |  |

> `matches[]` 保存回答當下的片段快照，而非指向片段文件的參照。影片其後被重新處理或刪除時，既有提問紀錄仍可重現當時的回答依據。

### DB-11 usage_logs

**資料檔陳述**：登入、觀看、開啟影片、提問、點擊精華片段與短影音腳本生成等事件的稽核歷史。

表8-2-13　`usage_logs` 欄位 Meta data

| Collection | `usage_logs` | 中文名稱 | 使用紀錄 | DB 編號 | DB-11 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `usage_logs` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `userId` → `users._id`；`courseId` → `courses._id` |  |  |  |  |
...
**資料檔陳述**：課程層級的常見問題快取與命中統計；網頁與 LINE 提問共用。

表8-2-14　`faqs` 欄位 Meta data

| Collection | `faqs` | 中文名稱 | 問答快取 | DB 編號 | DB-12 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `faqs` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`。快照：`matches[]`、`clip` |  |  |  |  |
...
**資料檔陳述**：LINE 帳號綁定用的一次性權杖。使用者在網頁取得權杖後貼到 LINE Bot 完成綁定。

表8-2-15　`line_bind_tokens` 欄位 Meta data

| Collection | `line_bind_tokens` | 中文名稱 | LINE 綁定權杖 | DB 編號 | DB-13 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | `mongoose.model` 第三個參數指定 `line_bind_tokens` | **timestamps** | 未啟用（自行宣告 `createdAt`） |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
**資料檔陳述**：站內通知，包含影片處理完成通知、管理員系統公告與短影音成品退回通知。

表8-2-16　`notifications` 欄位 Meta data

| Collection | `notifications` | 中文名稱 | 站內通知 | DB 編號 | DB-14 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `recipientId`、`createdBy` → `users._id`；`courseIds[]` → `courses._id`；`videoId` → `videos._id`；`scriptId` → `shortscripts._id` |  |  |  |  |

> 系統公告刻意不帶去重鍵，不參與 partial unique 索引；同一事件對同一收件者的重複通知則由該索引在資料庫層擋下。刪除影片時，service 會一併移除該影片的通知。

### DB-15 clips

**資料檔陳述**：片段對應的播放連結、重點與跳轉網址。屬 legacy／選用的快取層，不是正式的權威資料來源，也不是精華片段發布來源。

表8-2-17　`clips` 欄位 Meta data

| Collection | `clips` | 中文名稱 | 精華片段 | DB 編號 | DB-15 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `clips` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`segmentId`（字串）→ `video_segments_text.segmentId` |  |  |  |  |

### DB-16 conversations

**資料檔陳述**：網頁多輪問答的對話容器，一名學生在一門課程可有多個對話。

表8-2-18　`conversations` 欄位 Meta data

| Collection | `conversations` | 中文名稱 | 對話 | DB 編號 | DB-16 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `conversations` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
**資料檔陳述**：對話中的單則訊息（使用者提問或助理回覆）、引用來源快照與回覆關係。

表8-2-19　`messages` 欄位 Meta data

| Collection | `messages` | 中文名稱 | 訊息 | DB 編號 | DB-17 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `messages` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `conversationId` → `conversations._id`；`replyToMessageId` → `messages._id`（自參照）。快照：`sources[]` |  |  |  |  |

### DB-18 shortscripts

**資料檔陳述**：短影音腳本的選題依據、凍結證據、多版腳本與審核狀態。功能受 `SHORT_SCRIPT_AUTOMATION_ENABLED` 控制，預設關閉。

表8-2-20　`shortscripts` 欄位 Meta data

| Collection | `shortscripts` | 中文名稱 | 短影音腳本 | DB 編號 | DB-18 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`createdBy`、`versions[].reviewedBy` → `users._id`。快照：`sourceQuestions[]`、`evidence[]` |  |  |  |  |

> `evidence[]` 是快照而非參照：來源影片其後被刪除、重新處理或自課程移除，都不影響已凍結的證據內容，腳本引用因此始終可追溯。

### DB-19 shortassets

**資料檔陳述**：教師在系統外產製後上傳的短影音成品，記錄審核、AI 揭露與同意確認、YouTube 上架、可播放性同步與封存資料。YouTube 上架流程尚未 live 驗證。

表8-2-21　`shortassets` 欄位 Meta data

| Collection | `shortassets` | 中文名稱 | 短影音成品 | DB 編號 | DB-19 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 預設命名 | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `courseId` → `courses._id`；`sourceVideoId` → `videos._id`；`sourceScriptId`（搭配 `sourceVersionNo`）→ `shortscripts._id`；`reviewedBy`、`archivedBy`、`disclosure.confirmedBy`、`reviewHistory[].reviewedBy` → `users._id`。快照：`courseSnapshot` |  |  |  |  |
...
**資料檔陳述**：使用者在系統內提交的問題回報，以及管理員的處理狀態與備註。

表8-2-22　`feedbacks` 欄位 Meta data

| Collection | `feedbacks` | 中文名稱 | 問題回報 | DB 編號 | DB-20 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `feedbacks` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `userId` → `users._id`；`attachments[].attachmentId` → `feedbackattachments._id` |  |  |  |  |

### DB-21 feedbackattachments

**資料檔陳述**：問題回報截圖的影像本體，與回報文件分開存放，使回報列表查詢不必載入影像。

表8-2-23　`feedbackattachments` 欄位 Meta data

| Collection | `feedbackattachments` | 中文名稱 | 回報附件 | DB 編號 | DB-21 |
| --- | --- | --- | --- | --- | --- |
| **主鍵** | `_id`（ObjectId） | **集合名稱** | 明確指定 `feedbackattachments` | **timestamps** | 啟用 |
| **欄位名稱** | **資料型態** | **必填** | **預設值** | **中文說明** | **限制／索引** |
...
| `createdAt`、`updatedAt` | Date | 是 | 自動維護 | 建立與更新時間 | `timestamps: true` |
| **參照（應用層）** | `feedbackId` → `feedbacks._id` |  |  |  |  |
...
表8-2-24 列出 21 個 schema 宣告的所有一般索引（不含每個集合必有的 `_id` 索引）。欄位後的「-1」表示遞減排序。

表8-2-24　schema 宣告的一般索引總表
...
語意檢索由 MongoDB Atlas Vector Search 負責，採分集合設計：文字、上層與視覺向量各自獨立，不共用集合也不共用索引。舊版單一集合 `video_segments.vector_index` 視為 legacy。表8-2-25 為程式碼與設定檔中可查到的目標契約。

表8-2-25　向量檢索索引契約

| 集合 | 索引名稱（設定來源） | 向量欄位與維度 | 相似度 | filter 欄位 | 定義與建立方式 |
| --- | --- | --- | --- | --- | --- |
| `video_segments_text` | `QA_ATLAS_VECTOR_INDEX_NAME`；程式預設為空字串，`backend/.env.example` 為 `text_embedding_index` | `embedding`，3072 | cosine | `courseId`（ObjectId）、`videoId`（String） | backend 程式**沒有**建立此索引的腳本；定義僅記載於 `database/tools/setup/init_indexes.js` 的註解（需在 Atlas 網頁手動建立）與 `backend/docs/current-state.md`。2026-09-26 實查為 READY，維度 3072，filter 為 `courseId`、`videoId` |
...
| mongosh 初始化腳本 | `database/tools/setup/init_collections.js`、`init_indexes.js` | 16 個集合（含 Pipeline 集合）與其索引 | 撰寫時間早於部分 model，與 schema 有差異（見表8-2-27）；對共享 Atlas 屬寫入操作，需核准 |
...
表8-2-27　`database/tools/setup/init_indexes.js` 與 Mongoose schema 的索引差異
...
表8-2-28　AI Pipeline 與匯入工具的集合／產物邊界
...
7. **交付形式**：系上規範列出 MDF／LDF 資料庫檔，本專案使用 MongoDB，沒有對應的檔案格式。資料庫以 Mongoose schema、初始化腳本與示範資料（`npm run seed`）說明與重建，資料內容可用 `mongodump` 匯出。

### Atlas 實查結果（2026-09-26）

以下結果以唯讀方式查詢共享 MongoDB Atlas 取得，僅代表查詢當下的狀態；先前記載的歷史快照（例如 2026-08-02 的 16 個集合）已不適用。

表8-2-29　Atlas 唯讀實查結果

| 項目 | 實查結果 | 與本章契約的關係 |
| --- | --- | --- |
| 集合清單 | 共 24 個集合，已包含 `notifications`、`conversations`、`messages`、`avatars`、`feedbacks`、`feedbackattachments`、`videobatches`、`shortscripts`、`shortassets` | 符合 |
| 一般索引 | `video_segments_text`、`video_segments_parent`、`questions`、`users` 的索引鍵均包含表8-2-24 所列項目；`enrollments` 與 `clips` 另有 schema 未宣告的單欄索引（`studentId`、`courseId`） | 大致符合；多出的索引來自較早的初始化腳本，不影響查詢；unique 等選項未逐一比對 |
| `text_embedding_index` | READY、可查詢，維度 3072、cosine，filter 為 `courseId`、`videoId` | 符合 |
| `parent_embedding_index` | READY、可查詢，維度 3072、cosine，filter 僅有 `courseId`、`videoId` | 與表8-2-25 不同：程式定義另含 `generationVersion`、`isActive`，實際索引尚未包含；階層式檢索目前預設關閉，尚未受影響 |
| `video_embedding_index` | READY、可查詢，維度 3072、cosine，filter 為 `video_id` | 符合 |
| 片段鍵分佈 | `video_segments_text` 共 823 筆，`videoId` 皆為字串；`courseId` 皆缺值 | 符合「以 `videoId` 為範圍條件」的設計 |
| 混合集合 | `videos` 共 21 筆，皆為 app-owned，皆無 `videoId`／`video_id` 欄位；沒有 Pipeline metadata 文件 | 符合 |
| Leaf 向量契約 | 129 筆為 `gemini-embedding-2`、3072 維、世代 `text_search_generation_v2`；其餘 694 筆沒有 provider、model、維度與世代欄位 | 與正式環境查詢設定相符的僅 129 筆，其餘為舊世代資料，重新處理後才會更新 |

上述查證只使用唯讀帳號，未在共享 Atlas 上執行初始化腳本、`syncIndexes` 或任何寫入。
