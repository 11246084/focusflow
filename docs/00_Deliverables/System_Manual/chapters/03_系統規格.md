...
> 現況基準：本章於 2026-09-19 改寫，依下列來源查證：repository 程式碼與套件清單、`.github/workflows/deploy.yml`、正式站 `https://focusflow.ntub.edu.tw` 的回應標頭與 CORS 實測，以及經 ngrok 通道唯讀讀取的正式後端 `/health`。虛擬機的 CPU、記憶體與執行環境版本未能從 repository 取得，表3-2-4 標示為待填。本章 AI 協助紀錄見第 14 章 AI-DOC-20260919。
...
下列功能已有程式，但**目前未用於正式問答**：階層式（Parent／Child）檢索，正式環境的 `/health` 顯示為關閉；`video_segments_video` 影片畫面向量，`/health` 顯示尚未接入問答。Pipeline 另會產出音訊向量檔，問答流程也不使用。
...
| API 文件 | OpenAPI 3、Swagger UI（`/docs`） | 主要 API 的規格說明；統計、管理與部分端點尚未完整收錄 |
...
| 瀏覽器 | 仍有安全更新的 Chrome、Edge、Firefox 或 Safari；尚未建立各版本的相容性測試矩陣 |
...
- **本機影片在正式網域的播放依賴 YouTube**。前端以相對路徑 `/uploads/...` 播放本機影片，而正式網域的 nginx 不轉送此路徑；因此在正式站上，影片需要上傳到 YouTube 才能正常播放。這是依設定推論的結果，尚未以實際帳號操作驗證。
...
| API 規格 | OpenAPI 3、Swagger UI | 說明主要 API；統計、管理與部分端點尚未完整收錄 |
