...
> 本章依 2026-09-14 本機 `backend/src/models/`、Pipeline uploader 與 `database/tools/` 整理。這些是**程式碼宣告的資料契約**；未對 shared Atlas 執行讀取或寫入，故集合筆數、實際 index、Vector Search index 與資料品質均標示為未知，不能由本章推定為 production 狀態。
...
> **歷史圖／待重繪：**下圖未包含 Enrollment default-deny、Avatar、VideoBatch、FAQ、Notification、ShortAsset、Parent segment，且未區分 app-owned 與 pipeline metadata 的 `videos` 混合邊界。

![初評原稿第8章圖像：圖8-1-1-MongoDB集合關聯圖.png](../images/圖8-1-1-MongoDB集合關聯圖.png)

圖8-1-1　MongoDB 集合關聯圖（初評歷史圖，待重繪）
